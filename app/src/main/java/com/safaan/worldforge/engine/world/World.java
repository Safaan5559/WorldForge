...
public final class World {
    ...
    private Block[] chunk(int cx,int cz){
        ...
        a=new Block[CHUNK_X*HEIGHT*CHUNK_Z];
        Random r=new Random(seed ^ (cx*341873128712L) ^ (cz*132897987541L));
        for(int x=0;x<16;x++) for(int z=0;z<16;z++){
            int wx=cx*16+x,wz=cz*16+z;
            double n=Math.sin(wx*.055)*5+Math.cos(wz*.047)*4+Math.sin((wx+wz)*.017)*3;
            int h=Math.max(3,Math.min(70,34+(int)n));
            for(int y=0;y<HEIGHT;y++){
                Block b=Block.AIR;
                if(y<h-4)b=Block.STONE; else if(y<h-1)b=Block.DIRT;
                else if(y==h-1)b=(h<18?Block.SAND:Block.GRASS); else if(y<18)b=Block.WATER;
                a[idx(x,y,z)]=b;
            }
        }
    ...
    private long modifiedKey(int x,int y,int z){return (((long)x&0x1fffffL)<<42)|(((long)y&0x7fL)<<35)|((long)z&0x7ffffffffL);}
    public Block get(int x,int y,int z){
        ...
        Block m=modified.get(modifiedKey(x,y,z));
        return m!=null?m:chunk(cx,cz)[idx(lx,y,lz)];
    }
    public void set(int x,int y,int z,Block b){
        if(y<0||y>=HEIGHT)return;
        modified.put(modifiedKey(x,y,z),b);
        chunk(Math.floorDiv(x,16),Math.floorDiv(z,16))[idx(Math.floorMod(x,16),y,Math.floorMod(z,16))]=b;
    }
}
